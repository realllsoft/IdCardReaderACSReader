...
public final class SmartCardMessage {
    ...
    public DataBlock parseDataBlock(byte[] data) {
        ...
        if (data[0] != (byte)0x80) {
            Log.w(TAG, String.format("Invalid data type [%02x]", data[0]));
            return null;
        }

        dataBlock = new DataBlock();

        length = (((int)data[4] & 0xff) << 24) | (((int)data[3] & 0xff) << 16 ) | (((int)data[2] & 0xff) << 8) | ((int)data[1] & 0xff);
        ...
        dataBlock.status = data[7];
        dataBlock.error = data[8];
    ...
    public EscapeResponseBlock parseEscapeResponseBlock(byte[] data) {
        ...
        if (data[0] != (byte)0x83) {
            Log.w(TAG, String.format("Invalid escape type [%02x]", data[0]));
            return null;
        }
